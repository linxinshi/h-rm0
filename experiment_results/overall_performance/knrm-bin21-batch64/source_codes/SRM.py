...
class srm(nn.Module):
    ...
    def __init__(self, opt, weights=None):
        ...
        if weights != None:
            ...
            #self.word_emb.weight.data.normal_(-1,1)
            torch.nn.init.normal_(self.word_emb.weight)
        #print (self.word_emb)
        self.word_emb.requires_grad_(False) # for ablation study
        ...
        self.alpha = torch.nn.Parameter(torch.tensor(1.0,requires_grad=True,dtype=torch.float32,device=self.device)) #1.0
    ...
    def get_intersect_matrix(self, q_embed, d_embed, attn_q, attn_d):
        ...
        log_pooling_sum = torch.sum(pooling_sum, 1)
        #print ('lps2',log_pooling_sum.shape)
        return log_pooling_sum

    def forward(self, inputs_q, inputs_d, mask_q, mask_d, is_train=False):
        #print (inputs_q.device)
        assert self.word_emb.weight.requires_grad==False
