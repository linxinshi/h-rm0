...
class srm(nn.Module):
    ...
    def __init__(self, opt, weights=None):
        ...
        #tensor_alpha = torch.FloatTensor(torch.as_tensor(0.1))
    ...
    def get_intersect_matrix(self, q_embed, d_embed, attn_q, attn_d):
        ...
        #pooling_value=torch.sigmoid(self.beta*(trans_sim/(np.pi)-self.mu)/self.sigma)*attn_d
        #print (pooling_value.size())
        #pooling_value = torch.exp((- ((trans_sim - self.mu) ** 2) )) * attn_d          
        # no attn_d for pv2
        ...
        #pooling_sum = torch.exp((- ((trans_sim - self.mu.view(1,1,-1)) ** 2) / (self.sigma.view(1,1,-1) ** 2) / 2)) 
        #print ('ps',pooling_sum.shape)
        #temp_pv=torch.exp(torch.clamp(self.alpha,-0.5,0.5)*pooling_value)
        ...
        #temp_pv=torch.mean(torch.clamp(pooling_value,min=1e-10)**self.alpha,2)
        #print (temp_pv.size())
        #pooling_sum=(temp_pv**(torch.reciprocal(self.alpha)))
